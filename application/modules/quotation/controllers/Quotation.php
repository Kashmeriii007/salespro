...
class Quotation extends MX_Controller {
    ...
    public function add_quotation_to_invoice()
    {
        ...
        if ($this->form_validation->run() === true) {
            ...
            $total_saleamnt = $this->input->post('grand_total_price',TRUE);
            ...
            $tax_info = array(
                ...
                'VDate'          => $createdate,
                'COAID'          => 2114,
                'Narration'      => 'Value added TAX For Invoice NO - '.$invoice_id,
                'Debit'          => $paid_tax,
                'Credit'         => 0,
                'IsPosted'       => 1,
                'CreateBy'       => $createby,
            ...
            if (!empty($quantity)) {
                ...
                foreach ($multipaytype  as $multipaytype) {
                    ...
                    $paymethod = array(
                        ...
                        'VDate'          =>  $createdate,
                        'COAID'          =>  $multipaytype,
                        'Narration'      =>  'Paid amount for customer  Invoice No - '.$invoice_id.' customer -'.$cusifo->customer_name,
                        'Debit'          =>  $multipayamount[$k],
                        'Credit'         =>  0,
                        'IsPosted'       =>  1,
                        'CreateBy'       =>  $createby,
                        'CreateDate'     =>  $createdate,
            ...
            if (!empty($squantity) && $squantity[0] != '') {
                ...
                $i=0;
                foreach ($smultipaytype  as $smultipaytype) {
                    ...
                    $this->db->insert('acc_transaction',$spaymethod); 
                    $i++;
            ...
            for ($i = 0, $n   = count($serv_id); $i < $n; $i++) {
                ...
                if (!empty($service_qty)) {
                    ...
                    $this->db->insert('acc_transaction',$service_amnt);
                }
